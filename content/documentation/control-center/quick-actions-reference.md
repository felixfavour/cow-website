---
title: "Quick Actions Reference"
description: "Every action available in the Quick Actions Pane, grouped by what it does, with the plan it needs."
category: "The Control Center"
order: 6
lastUpdated: "2026-09-12"
---

Actions marked **Teams** require a Teams subscription. Teams-only actions stay visible in the Quick Actions Pane with an upgrade prompt rather than being hidden, so you can always see what's available.

## Content creation

| Action | What it does | Plan |
|--------|---------------|------|
| **Display Bible** | Select and open scriptures | Free |
| **Search Whole Bible** | Full text search of the scriptures | Free |
| **Display Hymns** | Find verses and chorus to all hymns | Free |
| **Transcribe Sermon** | Transcribe sermon and auto-suggest Bible slides | Teams |
| **Add Song** | Save songs to your personal library | Free |
| **Search song lyrics** | Find lyrics to any song, native too | Teams |
| **Song Setlist** | Group multiple songs in one styled slide | Free |
| **Create Text Slide** | Create slides with notes and more | Free |
| **Add Media** | Display image, video or audio media | Free |
| **Add Time Slide** | Display the current time on a slide | Teams |
| **Add Countdown Timer** | Engage your church with countdown | Teams |
| **Slide Templates** | Use pre-made, fancy slide templates | Teams |
| **Add YouTube Video** | Embed YouTube videos | Teams |
| **Add Vimeo Video** | Embed Vimeo videos | Teams |
| **Import Slides from PDF** | Turn a PDF into slides | Free |

## Live control

| Action | What it does | Plan |
|--------|---------------|------|
| **Go Live** | Open the live presentation window | Free |
| **Close Live Window** | Close the active presentation window | Free |
| **Open Stage Display** | Open the confidence-monitor screen for musicians and speakers | Free |
| **Promote Slide to Live** | Push the selected slide to the live output | Free |
| **Add Banners/Alert** | Notify your audience without disruption | Teams |
| **Remove Alert** | Remove current alert | Teams |

## Library, schedules and workspace

| Action | What it does | Plan |
|--------|---------------|------|
| **My Library** | Save your favorite songs and slides | Free |
| **Create New Schedule** | Start a whole new service project | Free |
| **Recent schedule** | Browse and switch between all your schedules | Free |
| **Select Slides** | Enter bulk selection mode | Free |
| **Refresh Slides** | Re-fetch slides from the server (force sync) | Free |
| **Upload Offline Slides** | Sync locally saved slides to the cloud | Free |
| **Invite to Workspace** | Invite teammates | Teams |

## App and account

| Action | What it does | Plan |
|--------|---------------|------|
| **Open App Settings** | Customise account, profile, slide and Bible settings | Free |
| **Shortcuts & Hotkeys** | Open the shortcuts modal | Free |
| **Toggle Dark Mode** | Switch theme | Free |
| **What's New** | Latest updates and release notes | Free |
| **Join Community** | Connect with other CoW users on WhatsApp | Free |
| **Upgrade Plan** | Move to Teams | Free |
| **Sign Out** | Sign out of your account | Free |

Individual settings pages are directly searchable too: *Profile Settings, Display Settings, Bible Version Settings, Microphone Settings, Slide Settings, Slide Background Settings, Overlay Settings, Blank Settings, Storage Settings, Subscription Settings, Other Settings.* See the [Settings reference](/docs/settings-and-shortcuts/settings-reference).
